  ...
  Widget _builder(BuildContext context, GameViewModel _vieModel) {
    return Expanded(
      child: _GameListContainer(
        _vieModel,
        child: ListView(
          children: _vieModel.state.list
              .map((game) => _GameListItem(game, _vieModel.selectHandler, _vieModel.theme))
              .toList(),
        ),
      ),
    );
...
class _GameListItem extends StatelessWidget {
  ...
  _GameListItem(this._game, this._selectHandler, this._theme);
